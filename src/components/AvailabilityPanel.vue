...
<template><section class="glass bottom-availability"><div class="panel-title"><h2>{{admin?'站点可用性与状态':'全部站点空闲情况'}}</h2><span>{{stations.length}} 站 · 空闲 / 总数</span></div><div v-if="admin" class="network-status"><span v-for="(label,i) in labels" :key="label"><i :style="{background:palette[i]}"></i>{{label}} <b>{{totals[i]}}</b></span></div><div class="station-status-grid"><button v-for="s in stations" :key="s.stationId" :class="{active:selectedId===s.stationId}" @click="$emit('select',s.stationId)" :aria-pressed="selectedId===s.stationId"><div><b>{{s.district}} <small>{{s.stationId.slice(-3)}}</small></b><strong>{{s.idlePiles}}<small> / {{s.pileTotal}}</small></strong></div><div class="status-segments" :title="admin?keys.map((k,i)=>`${labels[i]} ${s[k]}`).join(' · '):`空闲 ${s.idlePiles}/${s.pileTotal}`"><template v-if="admin"><i v-for="(k,i) in keys" :key="k" :style="{width:`${s.pileTotal?s[k]/s.pileTotal*100:0}%`,background:palette[i]}"></i></template><i v-else :style="{width:`${s.pileTotal?s.idlePiles/s.pileTotal*100:0}%`,background:palette[0]}"></i></div></button></div></section></template>
