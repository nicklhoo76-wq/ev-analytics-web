...
const target=ref<'load_kw'|'idle_pile_count'>('load_kw'),payload=ref<ModelEvaluationPayload|null>(null),loading=ref(true),failed=ref(false)
onMounted(async()=>{try{payload.value=await api.getModelMetrics()}catch{payload.value=null;failed.value=true}finally{loading.value=false}})
...
const stationCount=computed(()=>payload.value?.meta.stationCount??block.value?.perStation.length??0)
...
</template><p v-else>{{failed?'模型评估数据读取失败，请稍后重试。':'当前目标暂无模型评估指标。'}}</p></section></template>
