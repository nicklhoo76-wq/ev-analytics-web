...
<template><div class="station-map" aria-label="北京站点空间分布"><div class="map-grid" aria-hidden="true"></div><button v-for="s in nodes" :key="s.stationId" class="station-marker" :class="{active:selectedId===s.stationId,left:s.x>62,middle:s.x>45&&s.x<=62}" :style="{left:s.x+'%',top:s.y+'%'}" :aria-label="`${s.district} ${s.stationId}，空闲${s.idlePiles}个`" :aria-pressed="selectedId===s.stationId" @click="emit('select',s.stationId)"><span class="marker-dot"></span><span class="marker-label">{{s.district}}<small>{{s.stationId.slice(-3)}}站 · {{s.idlePiles}}空闲</small></span></button><p v-if="!nodes.length" class="quiet-empty">站点空间数据暂不可用</p><span class="map-direction">北 ↑　站点坐标分布　东 →</span></div></template>
