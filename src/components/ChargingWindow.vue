...
const props=defineProps<{prediction:PredictionData|null;flat?:boolean}>()
...
const nearestIndex=computed(()=>Math.max(0,points.value.findIndex(p=>p.predictedFreePiles!==null)))
const defaultIndex=computed(()=>props.flat?nearestIndex.value:bestIndex.value)
const activeIndex=computed(()=>hovered.value??pinned.value??defaultIndex.value)
