...
import { Activity, ArrowUpRight, Expand, LocateFixed, Pause, Play, RotateCcw, X, Zap } from 'lucide-vue-next'
...
<template>
 <div class="nexus" :class="{paused,passenger:!admin}" @mousemove="pointer">
  ...
  <header class="masthead">
   ...
   <div class="head-actions"><span class="city"><LocateFixed :size="14"/>北京</span><button @click="paused=!paused" :aria-label="paused?'播放动态效果':'暂停动态效果'"><Play v-if="paused" :size="16"/><Pause v-else :size="16"/></button><button @click="fullscreen" aria-label="切换全屏"><Expand :size="17"/></button></div>
  </header>
