...
const recommendations=computed(()=>{const items=[...(prediction.value?.points||[])].filter(p=>p.predictedFreePiles!==null);return (forecastFlat.value?items:items.sort((a,b)=>b.predictedFreePiles!-a.predictedFreePiles!)).slice(0,4)})
...
const nearestPrediction=computed(()=>prediction.value?.points.find(p=>p.predictedFreePiles!==null)||null)
const recommendedPoint=computed(()=>forecastFlat.value?nearestPrediction.value:best.value)
const bestLabel=computed(()=>recommendedPoint.value?hour(recommendedPoint.value.intervalEnd):'—')
const metrics=computed(()=>admin.value?[
 ...
 {label:'推荐充电时点',value:bestLabel.value,unit:'',sub:forecastFlat.value?`最近时点 · 各时点差异 ${freeAmplitude.value} 个`:'预计空闲数量最多'},
...
<template>
 <div class="nexus" :class="{paused,passenger:!admin,'theme-light':theme==='light','theme-dark':theme==='dark'}" @mousemove="pointer">
  ...
  <main class="command-grid" :aria-busy="loading">
   ...
   <section class="glass forecast-panel"><div class="panel-title"><h2>{{admin?'负荷趋势与预测':'当前与未来空闲对比'}}</h2><div class="horizons"><button v-for="h in ([1,6,24] as const)" :key="h" :class="{active:horizon===h}" @click="horizon=h">{{h}}h</button></div></div><label v-if="admin" class="threshold-control">预警阈值<input v-model.number="threshold" type="range" min="10" max="100" step="5" aria-label="负荷预警阈值"/><b>{{threshold}}%</b><small>装机容量</small></label><BaseChart :option="admin?loadChart:userComparison" :height="191" :aria-label="admin?'历史及未来负荷走势与预警阈值':'站点当前与未来空闲数量对比'"/><div class="chart-summary"><span v-if="admin">预计峰值 <b>{{peak?.toFixed(1)??'—'}} <small>kW</small></b></span><span v-else>当前与 {{horizon}}h 后的预计数量</span><span>推荐时点 <b :class="{compact:forecastFlat}">{{bestLabel}}</b></span></div><div v-if="forecastFlat" class="chart-note">该时域预测空闲变化仅 {{freeAmplitude}} 个，默认显示最近有效时点，推荐不具备区分度。</div><div v-if="admin&&capacity>0&&!thresholdInScale" class="chart-note">全网阈值 {{thresholdKw.toFixed(0)}} kW 远高于当前负荷，图中暂不绘制阈值线；预警按站点单独判定。</div><div v-if="!prediction&&!admin" class="quiet-empty">暂无该时段预测</div></section>
